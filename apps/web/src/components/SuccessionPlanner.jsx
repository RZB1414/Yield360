import { maritalRegimes } from '@yield-360/shared';
import { FormField } from './FormField.jsx';
import { SectionCard } from './SectionCard.jsx';
import { formatCurrency } from '../lib/formatters.js';

function inputClassName() {
  return 'w-full rounded-2xl border border-slate/10 bg-white px-4 py-3 text-slate outline-none transition focus:border-deep focus:ring-4 focus:ring-deep/10';
}
...
function SummaryRow({ label, value }) {
  return (
    <div className="grid grid-cols-[1.2fr_0.8fr] border-t border-slate/10">
      <div className="px-4 py-3 font-medium text-slate">{label}</div>
      <div className="px-4 py-3 text-right font-semibold text-slate">{value}</div>
    </div>
  );
}
...
export function SuccessionPlanner({ input, succession, onFieldChange }) {
  return (
    ...
    >
      <div className="rounded-[30px] border border-slate/10 bg-white p-6">
        <h3 className="font-display text-4xl text-slate">CHECKUP DE PLANEJAMENTO SUCESSORIO (Brasil)</h3>
        <p className="mt-2 text-sm text-slate/70">Gerado no app com os mesmos campos centrais mostrados na planilha de referencia.</p>

        <div className="mt-6 overflow-hidden rounded-[22px] border border-slate/10">
          <div className="bg-[#111111] px-4 py-3 font-semibold text-white">1) Dados e perguntas essenciais</div>
          <div className="grid gap-4 p-4 md:grid-cols-2 xl:grid-cols-3">
            <FormField label="Estado civil">
              <input className={inputClassName()} value={input.succession.maritalStatus} onChange={(event) => onFieldChange('succession.maritalStatus', event.target.value)} />
            </FormField>
        ...
        <div className="mt-6 overflow-hidden rounded-[22px] border border-slate/10">
          <div className="bg-[#111111] px-4 py-3 font-semibold text-white">2) Patrimonio para calculo</div>
        ...
        <div className="mt-6 overflow-hidden rounded-[22px] border border-slate/10 bg-[#fbfcfd]">
          <div className="bg-[#111111] px-4 py-3 font-semibold text-white">3) Calculo automatico</div>
